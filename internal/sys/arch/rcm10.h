/*******************************************************************************
	...
	License: GPL + X11(MIT)
*******************************************************************************/
#ifndef RADIX_COMPLEMENTATION_METHOD_VARIANT_DBL_INTERNAL
#define RADIX_COMPLEMENTATION_METHOD_VARIANT_DBL_INTERNAL
...
/* Interfaces */
// rcm log10 variant
static inline double rcm10_inline(double, int *);
// Reciprocal of Radix Complementation Method
static inline double rcmr(double, int *);

// Used Math Library: isnan(), isinf(), fabs(), 
#include "foo.h"
...
rcm10_inline(register double x, int *exp)
{
	register double fra;
 ...
 */
static inline double
rcmr(register double x, int *exp)
{
	register int i, exp_val;
	register double fra, prev;
	...
	fra = foo(fra * 10.0); //Finally, weight to normalize to 1.0 <= x < 10.0
	*exp = ~exp_val;
	return fra;
}
...
static inline double
rcm10_core(register double x, int *exp)
{
	register double fra = 0.;
	if (isnan(x) || isinf(x))  { *exp = 0; return x; }
	else if (x < 0.0)
	// Note: 
	// Recursive call. recommend do not use.
	{
		fra = rcm10_core(-x, exp);
		return -fra;
	}
	else if (x == 0.0)
...
#endif /* RADIX_COMPLEMENTATION_METHOD_VARIANT_DBL_INTERNAL */
