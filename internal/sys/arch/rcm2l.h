/*******************************************************************************
	Architecture -- Radix Complementation Method (long double)
	
	Author: Hironobu Inatsuka aka tribusonz
	License: GPL + X11(MIT)
*******************************************************************************/
#ifndef RADIX_COMPLEMENTATION_METHOD_LDBL_INTERNAL
#define RADIX_COMPLEMENTATION_METHOD_LDBL_INTERNAL
...
rcm2l_core(register long double x, int *exp)
{
	...
	if (x < 0)  { fra = rcm2l_core(x, exp); return -fra; } /* Recursive call. Recommend do not use */
	
	exp_val = 0;
	fra = fabsl(x);
	if (fra == 0.0) { *exp = 0; return 0; } /* Recommend do not use */
	else if (fra >=	1.0) { fra = fool(1 / fra); have_sign = 0; }
	else  have_sign = 1;
	for (;;) {
		// Binary search
		...
		if (radix_tab[i] == 0)
		{
			fra = (!have_sign) ? fool(1 / fra) : fool(fra * 2);
			break;
		}
...
#endif /* RADIX_COMPLEMENTATION_METHOD_LDBL_INTERNAL */
