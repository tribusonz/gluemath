...
*******************************************************************************/
#ifndef GML_EXP_DBL_INTERNAL
#define GML_EXP_DBL_INTERNAL
...
exp_core(register double x)
{
	...
	if (x != x)  return NAN;
	else if (x >= exp_min && x <= exp_max)
	{
		floorx = INT_FLOOR_INLINE(x); vfra = x - floorx;
		vexp = ur_ipow(E, floorx);
		if (vfra == 0.0)  return vexp;
		return vexp * expxt(vfra, 1);
	}
