...
*******************************************************************************/
#ifndef GML_EXPXT_DBL_INTERNAL
#define GML_EXPXT_DBL_INTERNAL
...
expxt(register double x, register double t)
{
	register double a, b, c, s;
	get_dbl_info();
	a = x * t; b = 1.0; c = 1.0; s = 1.0;
	do {
		b = b * a / c;
		if (b < dbl_info.epsilon)  break;
		s = s + b;
		c++;
	} while (1);
	return s;
