/*******************************************************************************
    Math Library -- Psi Function, Polygamma Function
*******************************************************************************/
#include <math.h>
#define N  8

#define B0  1  /* à»â∫ÇÕBernoulliêî */
#define B1  (-1.0 / 2.0)
#define B2  ( 1.0 / 6.0)
#define B4  (-1.0 / 30.0)
#define B6  ( 1.0 / 42.0)
...
psi(register double x)
{
	...
	v += ((((((((B16 / 16)  * w + (B14 / 14)) * w
	          ...
	          + (B8  /  8)) * w + (B6  /  6)) * w
	          + (B4  /  4)) * w + (B2  /  2)) * w + 0.5 / x;
	return p_log(fabs(x)) - v;
}

/* $\psi^{(n)}(x)$ */
double
polygamma(register int n, register double x)
{
	register int k;
	register double t, u, v, w;
