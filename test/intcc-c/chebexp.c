...
void
chebexp(
  integrand f, double a, double b, double eps,
  int lenc, double *c, double *err)
{
	void ddct(int, double, double, double *);
	int j, k, l, n;
	double esf, ba, cos2, sin2, wi, ss, x, y, t, h,
	       eref = 0, erefh = 0, errh = 0;
	...
	do {
		...
		if (n == 4)
		{
			eref = 0.25 * (fabs(c[lenc]) + fabs(c[lenc - 1]) + 
		...
		h *= 0.5;
		errh = esf * *err;
		*err = h * (0.5 * fabs(c[lenc - n]) + fabs(c[lenc - n + 1]));
