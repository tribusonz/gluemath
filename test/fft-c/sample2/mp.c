...
mp_load_1(int n, int radix, int out[])
{
	...
	out[1] = 0;
	out[2] = 1;
	for (j = 3; j <= n + 1; j++)  out[j] = 0;

}


void
mp_copy(int n, int radix, int in[], int out[])
{
	int j;
	
	for (j = 0; j <= n + 1; j++)  out[j] = in[j];
}
...
mp_add(int n, int radix, int in1[], int in2[], int out[])
{
	...
	else
	{
		...
		outexp -= expdif;
		outsgn *= in1[0];
		if (expdif == n)  outsgn = 0;
	}
	if (outsgn == 0)  outexp = 0;
	out[0] = outsgn;
	out[1] = outexp;
}
...
void
mp_unsgn_imul(
  int n, double dradix, int in1[], double din2, int out[])
{
	int j, carry, shift;
	double x, d1_radix;
...
void
mp_mul(
  int n, int radix, int in1[], int in2[], int out[], 
...
void
mp_squ(
  int n, int radix, int in[], int out[], int tmp[], 
...
void
mp_squh(
  int n, int radix, int in[], int out[], 
...
void
mp_squh_use_in1fft(
  int n, int radix, double inoutfft[], int out[], 
...
void
mp_mul_i2d(
  int n, int radix, int nfft, int shift, 
  int in[], double dout[])
{
...
void
mp_mul_cmul(int nfft, double din[], double dinout[])
{
	int j;
	double xr, xi, yr, yi;
...
void
mp_mul_cmuladd(
int nfft, double din1[], double din2[], double dinout[])
{
	int j;
	double xr, xi, yr, yi;
...
void
mp_mul_csqu(int nfft, double dinout[])
{
	int j;
	double xr, xi;
...
void
mp_unexp_d2mp(int n, int radix, double din, int out[])
{
	int j, x;
...
double
mp_unexp_mp2d(int n, int radix, int in[])
{
	int j;
...
void
mp_fprintf(int n, int log10_radix, int in[], FILE *fout)
{
	int j, k, x, y, outexp, shift;
	char out[256];
