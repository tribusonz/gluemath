/*******************************************************************************
	GLUE-Math test suite: Ceiling function
*******************************************************************************/
...
$ gcc -Wall -Wextra -O3 -o bin/ceil test/ceil.c
...
Value:  9456.493844250217080 ->  9457.000000000000000
...
Value:  5695954.370530229061842 ->  5695955.000000000000000

value, ceil (long double)
Value: -0.219365568831562996 -> -0.000000000000000000
Value:  10.309692677110433578 ->  11.000000000000000000
...
Value:  55.416685514152050018 ->  56.000000000000000000
Value:  359.475884872488677502 ->  360.000000000000000000
Value:  65.063979097642004490 ->  66.000000000000000000
Value: -1110.054836086928844452 -> -1110.000000000000000000
Value:  793.421177040785551071 ->  794.000000000000000000
Value:  80957.105812511872500181 ->  80958.000000000000000000
...
Value:  46216.201224522665143013 ->  46217.000000000000000000
...
main(void)
{
	...
	puts("Test Suite:");
	puts("  Ceiling Function");
	puts("");

	puts("value, ceil (double)");
	for (volatile int i = 0; i < 5; i++)
	{
		const double x = (0.5 - rnd()) * (irnd() / 123456789UL); // Iterator Constant
		printf("Value: % *.*f -> % *.*f\n",
		       DBL_DIG, DBL_DIG, x, DBL_DIG, DBL_DIG, ceil(x));
	}
	for (volatile int i = 0; i < 5; i++)
	{
	...
	puts("");
	puts("value, ceil (long double)");
	for (volatile int i = 0; i < 5; i++)
	{
		const long double x = (0.5 - rnd()) * (irnd() / 123456789UL); // Iterator Constant
		printf("Value: % *.*Lf -> % *.*Lf\n",
		       LDBL_DIG, LDBL_DIG, x, LDBL_DIG, LDBL_DIG, ceill(x));
	}
	for (volatile int i = 0; i < 5; i++)
	{
	...
	puts("");
	puts("value, ceil (float)");
	for (volatile int i = 0; i < 5; i++)
	{
		const float x = (0.5 - rnd()) * (irnd() / 123456789UL); // Iterator Constant
		printf("Value: % *.*f -> % *.*f\n",
		       FLT_DIG, FLT_DIG, x, FLT_DIG, FLT_DIG, ceilf(x));
	}
	for (volatile int i = 0; i < 5; i++)
	{
