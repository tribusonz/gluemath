 ...
 * */
static void
G_cfrac_lower(double *Gcfrac, double p, double x)
{
	...
	f=an/bn; c=an/DBL_MIN; d=1./bn; n=2;
	do {
		k=n/2; an = ((n&1) ? k : -(p-1+k))*x; bn++;
		d=an*d+bn; if (d == 0) d=DBL_MIN;
		c=bn+an/c; if (c == 0) c=DBL_MIN;
		d=1.0/d; del=d*c; f*=del; n++;
	}
	while ((fabs(del-1.0) >= DBL_EPSILON) && (n < ITMAX));
	*Gcfrac = f;
 ...
 * */
static void
G_ibp(double *Gibp, double p, double x)
{
...
#else
	gln = lgamma(p); t = fabs(x); tt = 1./(t*t);
#endif
	odd = ((int)p)%2;
	...
	do {
		...
		del = c*(t-d);
		s += del; l++;
		stop = (fabs(del) < fabs(s)*DBL_EPSILON);
 ...
 * */
static void
G_cfrac_upper(double *Gcfrac, double p, double x)
{
	double c,d,del,f,an,bn;
	int i,n;
	char t;
	...
	if(t) { // b{1} is non-zero
		...
		an=-(1-p); bn=x+3.0-p;
		f=an/bn; c=an/DBL_MIN; d=1./bn; n=3;
	}
	i = n-1;
	do {
		an = -i*(i-p); bn += 2.0;
 ...
 * */
matherrstatus_t
G_func(double *G, double p, double x)
{
...
double *rho, *sigma, x, y, mu, p;
{
	...
	*sigma = -mu*y + (p-1)*log(y);
	...
	/* loop for n > 0 */
	relneeded = DBL_EPSILON/TOL_ROMBERG;
	adr0 = 0;
	n = 1;
 ...
 * */
matherrstatus_t
deltagammainc(double *rho, double *sigma, char *method, double x, double y, double mu, double p)
{
	double mA, mB, mx, my, nA, nB, nx, ny, gln;

	/* deal with particular cases */
	if (x==y){ *rho = 0.; *sigma = -INFINITY; (*method) = -1; return E_MATHNOERR; }
#if 0
	if (x==0. && isinf(y)){ *rho = 1.; Gammaln(sigma,p); (*sigma) = (*sigma)-p*log(mu); (*method) = -2; return E_MATHNOERR; }
#else
	if (x==0. && isinf(y)){ *rho = 1.; *sigma = lgamma(p); (*sigma) = (*sigma)-p*log(mu); (*method) = -2; return E_MATHNOERR; }
#endif
...
double
deltagammaincsolver(double rho, double sigma)
{
	if (rho==0 && isinf(sigma) && sigma < 0) return 0.0;
	return rho*exp(sigma);
}

matherrstatus_t
deltagammainccheck(double x, double y, double mu, double p, char *flag)
{
	*flag = '\0';
	if (x != x)
		{ *flag = 'x'; return E_DOMNANARG; }
	else if (y != y)
		{ *flag = 'y'; return E_DOMNANARG; }
	else if (mu != mu)
		{ *flag = 'm'; return E_DOMNANARG; }
	else if (p != p)
		{ *flag = 'p'; return E_DOMNANARG; }
	else if (x < 0)
		{ *flag = 'x'; return E_DOMNEGARG; }
	else if (y < 0)
		{ *flag = 'y'; return E_DOMNEGARG; }
	else if (x > y)
		return E_DOMGT;
	else if (isinf(y) && mu < 0)
		{ *flag = 'y'; return E_DOMNOSOLVE; }
	else if (p <= 0)
		return E_DOMZERONEGARG;
	else if ((mu < 0) && (p != floor(p)))
		{ *flag = 'p'; return E_DOMNOSOLVE; }
	else if (mu == 0)
		return E_DOMZERO;
	return E_MATHNOERR;
}
...
matherrstatus_t
G_funccheck(double p, double x, char *flag)
{
	*flag = '\0';
	if (x != x)
		{ *flag = 'x'; return E_DOMNANARG; }
 ...
 * integer value
 * */
// FIXME: does not support 0 < x < 1 of this routine;
matherrstatus_t
scientific_notation(double *a, double *b, double rho, double sigma)
{
	...
	l10 = log(10);
	delta = log10(rho)+sigma/l10; // sigma == inf then NaN
	*b = floor(delta);
	*a = exp((delta-(*b))*l10);
	return E_MATHNOERR;
}
